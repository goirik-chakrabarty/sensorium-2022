...
class SuperLoss(nn.Module):
    ...
    def forward(self, loss):
        ...
        self.tau = (self.tau * (self.counter - 1) + l_i.mean()) / self.counter  # update tau with the mean of l_i
...
def standard_trainer(
    ...
    **kwargs
    ...
    def full_objective(model, dataloader, data_key, *args, **kwargs):
        ...
        loss = loss_scale * criterion(model(args[0].to(device), data_key=data_key, **kwargs),args[1].to(device),)
        if True:
            loss = superloss(loss)
